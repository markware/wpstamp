...
var wpStamp =  {
   ...
   state : [],
    ...
    reShape: function(currentWidth, currentHeight) {
        ...
        /*
   console.log("reshape:"+ratio);
console.log(xtrawidth,newHeight);
        drawingArea.css({
        ...
        $('.canvas-container').css({
            ...
            wpStamp.canvas.setHeight(newHeight);
             ...
             //      wpS)tamp.canvas.calcOffset().renderAll();
...
wpStamp.canvas.clipTo = function(ctx) {
  var bounds = new fabric.Rect({
                            
                                left:  Number($('#tcanvas').attr('data-left')),
                                top: Number($('#tcanvas').attr('data-top')),
                                width: Number($('#tcanvas').attr('data-width')),
                                height:  Number($('#tcanvas').attr('data-height')),
                                fill:'transparent',
                                rx:20,ry:20
                            });
...
wpStamp.realArea();
    /*      
                wpStamp.bounds = new fabric.Rect({
                    originX: 'left',
                    originY: 'top',
                    left: $('#tcanvas').attr('data-left'),
                    top: $('#tcanvas').attr('data-top'),
                    width: $('#tcanvas').attr('data-width'),
                    height:  $('#tcanvas').attr('data-height'),
                    fill: 'transparent', 
                    selectable: false
                });
            ...
            //   wpStamp.canvas.add(wpStamp.bounds);
        ...
        $('[title]').tooltipster({offsetY:'20px'});
    ...
    layersPlugin:  {
            ...
            load: function() {
                ...
                if( wpStamp.getObjectById(id)!=false ){

                 }
                  var layers= $("#layers");
                  layers.find('.nothing').remove();
                  var upTool='<a class="fa-button bring-to-front" data-tooltip="Bring to Front"><em class="material-icons">keyboard_arrow_up</em></a>';
                  var downTool='<a class="fa-button send-to-back" data-tooltip="Send to Back"><em class="material-icons">keyboard_arrow_down</em></a>';
                  var removeTool='<a class="fa-button remove-selected" data-tooltip="Delete selected item"><em class="material-icons">delete</em> </a>'
                  layers.prepend('<li id="' + id + '" data-id="' + id + '"><em class="material-icons">' + icon + '</em>' + type+ " " + id + '<div class="small-tools">'+upTool+downTool+removeTool+'</div></li>');
 ...
 jsa.objects[0].radius=jsa.objects[0].radius*scale*wpStamp.ratio;
   ...
   /* 
              ...
              //  console.log(jsa.objects[0],scale);
              var bounds =  new fabric.Rect(jsa.objects[0]);



              //  console.log(jsa.objects[0].top,  jsa.objects[0].left, jsa.objects[0].width*jsa.objects[0].scaleX,jsa.objects[0].height*jsa.objects[0].scaleY)

                //        console.log(rleft,rtop,rwidth,rheight);
                     /*   var bounds = new fabric.Rect({
                                            left:  rleft * wpStamp.ratio,
                                            top: rtop * wpStamp.ratio,
                                            width: rwidth * wpStamp.ratio,
                                            height:  rheight * wpStamp.ratio,
                                            fill:'transparent',
                                            stroke:'0'
                                        });*/ 
    ...
    changeProduct: function(productId) {
     ...
     try{
        console.log(wpStamp.allProducts[productId]);
         ...
         var real_a = JSON.parse(real_drawing);
...
if(wpStamp.side=="back"){
  ...
  $('.frontside,.backside,.helper,#drawingArea').attr({
    ...
    /*   $("#font-family").change(function() {
            var activeObject = wpStamp.canvas.getActiveObject();   console.log(activeObject.type)

            if (activeObject && (activeObject.type === 'text' || activeObject.type === 'curvedText' )) {

                activeObject.set('fontFamily',$(this).val().replace(/\+/g, ' ') ? $(this).val().replace(/\+/g, ' ') : 'Arial');
                ...
                success: function(data, textStatus, XMLHttpRequest) {
                    ...
                    if (data.images.length > 0) {
                        ...
                        $(".photo-list,.product-list").niceScroll();
                          ...
                          $('#pagination').html("<li class='nothing active'></li>");
     ...
     $('#spacing')[0].noUiSlider.updateOptions({
        ...
        $('.designer-container h1').on('click', function(e) {
            ...
            $('.panels').removeClass('hide');
                ...
                setTimeout(function(){ $('#' + id).addClass('collapse animated slideInLeft');},10);
...
$('.designer-container h1').on('click', function(e) {
    e.preventDefault();
            ...
            $('#' + id).children('.heading').html("<button class='back-menu'><em class='material-icons'>arrow_back</em></button><h2>"+text+"</h2>");

});
$('body').on('click','.heading >.back-menu',function(){
var that=$(this);
    $('.my-tools').removeClass('amimated slideOutLeft').addClass('animated slideInLeft');
    $('.panels').removeClass('animated slideInRight').addClass(' animated slideOutRight'); 
    setTimeout(function(){ 
        ...
        jQuery.ajax({
            ...
            success: function(data, textStatus, XMLHttpRequest) {
                ...
                $.each(data, function(c, media) {
                    media_categories += '<li><a data-name="' + media.name + '" data-cat="' + media.id + '" href="#' + media.name + '">' + media.name + '</a></li>';
   ...
   // $(canvas.getElement().parentNode).on('mousewheel', function(e) {
      ...
      wpStamp.zoom=newZoom;
        ...
        $('.t-container').css({
            ...
            'width': canvas.getWidth() * canvas.getZoom() + 'px',
            'height':  canvas.getHeight() * canvas.getZoom() + 'px'
        });
         //wpStamp.zoomCanvas();
   ...
   // $(canvas.getElement().parentNode).on('mousewheel', function(e) {
      ...
      wpStamp.zoom=newZoom;
        ...
        $('.t-container').css({
         ...
         //wpStamp.zoomCanvas();
        console.log( {'top':  canvas.viewportTransform[4]+ 'px',
            'left':  canvas.viewportTransform[5]+ 'px',
            'width': canvas.getWidth() * canvas.getZoom() + 'px',
            'height':  canvas.getHeight() * canvas.getZoom() + 'px'})
       wpStamp.realArea(canvas.getZoom());
    ...
    addLibrary: function(url, id) {
        ...
        uploadWrap.find('.nothing').remove();
...
*/
                    ...
                    FB.api("/me", {
                        ...
                        FB.api({
                            ...
                            function(data) {
                                ...
                                $('.user-list').attr('id', "facebook").append(imglist);
    ...
    sendAjax: function(color, type, imgf, imgb, front, back) {
        ...
        $("#orderit-ajax").html('<em class="material-icons">timer</em>');
